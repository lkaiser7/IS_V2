...
LoadProjectionManually<-function(bm_proj){
  ...
  if(bm_proj@proj@inMemory){
    ...
    # otherwise load projection data manually from project path
  }else{
    ...
    if(length(filesToLoad)){
      ...
      # otherwise load projection data manually from a different path
    }else{
...
.testnull<-function(object, Prev = 0.5 , dat){
  # load response predictors if already done
  ...
  # calculate response predictors if does not exist
  if(!exists('pred')){
    if(inherits(dat,'Raster')){
...
setMethod('.Prepare.Maxent.Proj.WorkDir', signature(Data='data.frame'),
          def = function(Data, xy, species.name = ".", proj.name = ".", silent = FALSE){
            ...
            if(is.null(xy)) xy<-matrix(1, nrow = nrow(Data), ncol = 2, 
            ...
            write.table(Proj_swd, file = file.path(species.name, proj.name, maxentWDtmp, 'Proj_swd.csv'), 
                        quote = FALSE, row.names = FALSE, col.names = TRUE, sep = ",")
          })
# set methods for function to run for raster stack 
              ...
              if(! file.exists(file.path(species.name, proj.name, maxentWDtmp, 'Proj',
                ...
                if(grepl(".asc", filename(raster::subset(Data, l, drop = TRUE)))){
                  ...
                  writeRaster(raster::subset(Data, l, drop = TRUE), 
                              filename = file.path(species.name, proj.name, maxentWDtmp, 
                                                   'Proj', paste0(l, '.asc')),
                              format = 'ascii', overwrite = TRUE)        
                }                
              }else{
                if(!silent) cat("\n", file.path(species.name, proj.name, maxentWDtmp,'', 
                                                paste0(l, '.asc')), 'already created !')
              }              
            }
          })

# set class for maxent biomod2 model type
...
# set method for maxent biomod2 model 
...
# function for maxent projections 
.predict.MAXENT_biomod2_model.RasterStack<-function(object, newdata,  ...){
  ...
  # sign posting for running maxent 
  cat("\n\t\tRunning Maxent...")
  ...
  # return projections from maxent
  return(proj)
}

# function for maxent predictions 
