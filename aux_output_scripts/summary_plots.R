...
#COMMENTED THIS ALL OUT SINCE CODE IS SPECIFIC TO SPECIFIC MODEL TYPES USED, NOT GENERALIZED
# ### CURVES ###
# # library (ggplot2)
...
#     
#     # loop ticker
#     current_sp_nm=replace_spp_names(all_sp_nm[r])
#     print(all_sp_nm[r])
#     
#     # select species records c("global_notHI", "regional_HI", "nested_HI")
#     g_sp<-cbind(model = "global_notHI", g_eval[which(g_eval[2] == all_sp_nm[r]),])
#     l_sp<-cbind(model = "regional_HI", l_eval[which(l_eval[2] == all_sp_nm[r]),])
#     n_sp<-cbind(model = "nested_HI", n_eval[which(n_eval[2] == all_sp_nm[r]),])
#     
...
#       x_max<-as.numeric(m_sp[1, -1:-4])
#       max_mean<-mean(x_max, na.rm=T)
#       x_gbm<-as.numeric(m_sp[2, -1:-4])
...
#            main = paste(current_sp_nm, m_name, eval_stat, "Model Evaluation Metrics"))
...
#   
#   # select species
#   g_sp<-g_var[which(g_var[2] == all_sp_nm[r]),]
#   l_sp<-l_var[which(l_var[2] == all_sp_nm[r]),]
#   n_sp<-n_var[which(n_var[2] == all_sp_nm[r]),]
#   
#   # combine records to loop
...
#   #   m_sp<-all_sp[[m]]
#   #   
#   #   model_to_run = models_to_run[1]
#   #   for (model_to_run in models_to_run){
#   #     mod_col_names=grep(model_to_run, names(g_sp), value = T)
#   #   }
#   #   # set up data variables 
